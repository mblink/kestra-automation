...
- Emits `WATCHING`, `CONFIG ERROR` (from `errors[]`), `STEP FAILED <name> … log: repos/<id>/logs/<n>/<stepId>` per failing step, then one terminal line: `BUILD PASSED` / `BUILD FAILURE|ERROR|DECLINED` / `BUILD CANCELED` / `BUILD BLOCKED` / `STALLED` / `QUEUED` / `NO PROGRESS` / `NO PIPELINE`. Exit 0 green, 1 red, 2 nothing ran.
- Pass `--sha $(git rev-parse HEAD)` after pushing a fix, or the previous failure is reported again. A `killed` pipeline gets one extra poll before being called canceled.
